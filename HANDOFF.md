...
- [x] 🟠 **`POST /api/customers` debe ser atómico.** Si falla la generación de las URLs de billetera, el `Customer` y el `Pass` ya quedaron creados, y el reintento responde `isNew: false` **sin URLs**: el cliente queda registrado pero sin tarjeta y sin forma de obtenerla.
- [x] 🟠 **Recuperar un pase perdido.** Flujo OTP por SMS implementado en backend y frontend (2026-09-25). Endpoint de solicitud y verificación con rate limiting y hash seguro. La pantalla `JoinSuccess` y `Join` permiten solicitar el código y revelar los enlaces de billetera de forma segura.
...
- [x] 🟠 **Verificación del cliente (OTP por SMS).** Implementado en `verifyRecoveryCode` (2026-09-25). Si un cliente legacy no tiene teléfono o RUT registrado, la verificación exitosa del código OTP valida y completa los datos faltantes de manera segura sin permitir suplantación.
...
- [x] 🟡 **Borrado de datos personales** (Ley 19.628, §7.2). Implementado en backend y frontend (2026-09-25). Endpoints `DELETE /api/customers/:customerId?merchantId=...` (para dueños con RLS/verificación de rol OWNER) y `DELETE /api/customers/:customerId` (global). Flujo self-service con OTP por SMS (`/api/customers/deletion/request` y `/verify`). Botón de acción con diálogo de confirmación `ConfirmDialog` en la tabla de clientes del panel admin (`Customers.tsx`).
